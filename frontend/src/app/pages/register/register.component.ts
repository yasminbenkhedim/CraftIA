...
@Component({
    ...
    <div class="auth-page">
      ...
      <div class="auth-card">
        ...
        <div class="head">
          ...
          <p class="sub">Spin up your first agent crew in under a minute.</p>
        </div>
        ...
        <form class="form" (ngSubmit)="onSubmit()" novalidate>
          ...
          <label class="field">
            ...
            <input
              ...
              [disabled]="isSubmitting"
            />
            <span class="hint" *ngIf="password && password.length < MIN_PASSWORD">
              {{ MIN_PASSWORD - password.length }} more character(s) needed
            </span>
          </label>
          ...
          <button type="submit" class="submit" [disabled]="!canSubmit">
            ...
            <ng-container *ngIf="isSubmitting"><span class="spinner"></span>Creating…</ng-container>
          </button>
        </form>

        <div class="foot">
          Already have an account? <a routerLink="/login" [queryParams]="{ returnUrl: returnUrl }">Sign in</a>
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .brand-sub { font-size: 9px; letter-spacing: .16em; color: var(--cf-text-dim); }
    ...
    .hint { font-size: 11px; color: var(--cf-warn-soft); }
    .field input {
      height: 42px;
      padding: 0 13px;
      border-radius: 10px;
    ...
    .foot { text-align: center; font-size: 12.5px; color: var(--cf-text-muted); }
  `]
})
export class RegisterComponent {
  ...
  get canSubmit(): boolean {
    return !this.isSubmitting
      && !!this.fullName.trim()
      && !!this.email.trim()
      && this.password.length >= this.MIN_PASSWORD;
  }
  ...
  onSubmit() {
    ...
    this.auth.register(this.fullName.trim(), this.email.trim(), this.password).subscribe({
      next: () => {
        // register returns a token, so the user is already signed in.
      ...
      error: (err) => {
        ...
        this.errorMessage = err.error?.detail
          || 'Could not create the account. Please check the backend connection.';
      }
    });
  }
}
